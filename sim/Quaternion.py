...
def invCoordinateTransformation(gripperCenterAfterTransform, gripperOrientationAfterTransform, cameraTranslation,
                                cameraRotation):
    rotation = Quaternion(w=cameraRotation[0], x=cameraRotation[1], y=cameraRotation[2], z=cameraRotation[3])
...
class Quaternion(object):
    ...
    def __add__(self, quaternion2):
        if isinstance(quaternion2, Quaternion):
            return Quaternion(self.w + quaternion2.w, self.x + quaternion2.x, self.y + quaternion2.y,
                              self.z + quaternion2.z)
        else:
